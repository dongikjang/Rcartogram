...
R_predict(SEXP obj, SEXP r_x, SEXP r_y, SEXP r_dims)
{
    ...
    SEXP r_ans = PROTECT(allocVector(VECSXP, 2));

    n = Rf_length(r_x);
    
    SEXP sx_ans_ptr = allocVector(REALSXP, n);
    SEXP sy_ans_ptr = allocVector(REALSXP, n);
    x_ans_ptr = REAL(sx_ans_ptr);
    y_ans_ptr = REAL(sy_ans_ptr);
	...
	x_ans_ptr[i] = (1. - dx)*(1. - dy)*gridx[ix*dims[0] + iy] + dx*(1. - dy)*gridx[(ix+1*dims[0]) + iy] + 
                       (1. - dx)*dy*gridx[ix*dims[0] + iy+1] + dx*dy*gridx[(ix+1)*dims[0] + iy+1];

	y_ans_ptr[i] = (1. - dx)*(1. - dy)*gridy[ix*dims[0] + iy] + dx*(1. - dy)*gridy[(ix+1)*dims[0] + iy] + 
                       (1. - dx)*dy*gridy[ix*dims[0] + iy+1] + dx*dy*gridy[(ix+1)*dims[0] + iy+1];

    }

    SET_VECTOR_ELT(r_ans, 0, sx_ans_ptr);
    SET_VECTOR_ELT(r_ans, 1, sy_ans_ptr);
    UNPROTECT(1);
